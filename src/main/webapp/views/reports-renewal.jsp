...
function runSubmissionReport(){
	var brandName=$('#brandName').val();
	var StoreName=$('#StoreName').val();
	document.location.href = "runSummaryReport.htm?store="+StoreName+"&&brandName="+brandName;
...
function runRenewalReport(){
	var brandName=$('#brandName').val();
	var StoreName=$('#StoreName').val();
	document.location.href = "renewalReportDownload.htm?store="+StoreName+"&&brandName="+brandName;
	...
	<div class="content_wrapper no-sidebar"> 
		...
		<div class="content" id="id_content">
			...
			<!-- 	<a href="" class="ico_pdf">PDF</a> -->
				<a href="#" class="ico_xls" onclick="runRenewalReport()">RENEWAL EXCEL</a>
				<a href="#" class="ico_xls" onclick="runSubmissionReport()">SUBMISSION EXCEL</a>
				...
				<div class="box box_red_title renewal"> 
					...
					<div id="id_report_renewal">
						<table width="100%" class="grid grid_23">
							...
							<tbody>
							<core:forEach items="${renewalReports }" var="client">
							<tr class="odd">
									<td><div>${client.store}</div></td>
									<td><div>${client.brandName}</div></td>
									<td><div>${client.businessName}</div></td>
									<fmt:formatDate value="${client.renewalDate}" pattern="MM/dd/yyyy" var="acxiomRenewalDate"/>
									<td class="td_01"><div>${acxiomRenewalDate}</div></td>
									<fmt:formatDate value="${client.renewalDate}" pattern="MM/dd/yyyy" var="factualRenewalDate"/>
									<td class="td_01"><div>${factualRenewalDate}</div></td>
									<fmt:formatDate value="${client.renewalDate}" pattern="MM/dd/yyyy" var="infogroupRenewalDate"/>
									<td class="td_01"><div>${infogroupRenewalDate}</div></td>
									<fmt:formatDate value="${client.renewalDate}" pattern="MM/dd/yyyy" var="localezeRenewalDate"/>
