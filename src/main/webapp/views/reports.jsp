...
if(uploadreportpopup!=''){
		 ...
		 if(selRptId==1){
					...
					//$("#searchbutton").show();
					 ...
					 if(selRptId==15){
							...
							$("#startdate").hide();
							$("#enddate").hide();
							$("#runDirectoryPerfLocationReport").show();
							$("#runReport").hide();
							$("#durationDrop").show();
					} 
					 if(selRptId==16){							
	...
	function rungmbMonthlyReport(){
		var brandname=$('#clientName').val();
		var gmboption = $("#gmboption").val();
		//var store = $("#storevalue").val();
		...
		if(brandname == ''){
			...
			document.location.href = "runGMBMonthlyReport.htm?brand="+brandname+"&&type="+gmboption;
		}
	}
	
	function runDirectoryPerfLocationReport(){
		var brandname=$('#clientName').val();
	...
	function runDirectoryPerfReport() {
		var brandname=$('#clientName').val();
		var duration=$('#duration').val();
	...
	function coverageLocationReport(){
		var brandname=$('#clientName').val();
		var duration=$('#duration').val();
		...
		if(brandname == '' || store== ''){
			...
			document.location.href = "coverageLocationReport.htm?store="+store+"&&brand="+brandname+"&&duration="+duration;
		}
	}
	...
	function coverageReport() {
		...
		if(brandname == ''){
			...
			document.location.href = "coverage.htm?b="+ brandname + "&&d=" + duration;
		}
	}
	...
	function accuracyReport() {
		...
		if(brandname == ''){
			...
			document.location.href = "accuracy.htm?b="+brandname+"&&d="+duration+"&&s="+store;
		}
	}
	...
	function rankingReport(){
		...
		if(brandname == ''){
			...
			document.location.href = "ranking.htm?b="+brandname+"&&d="+duration;
		}
	}
...
<script type="text/javascript">
 ...
 $(document).ready(
		 function() {
		  ...
		  /*$('#gmboption').change(function() {
			  var gmboption = $("#gmboption").val();
			  if(gmboption=='location'){
				$("#storeDrop").show();
				 $("#stateDrop").hide();
			  }
			  if(gmboption=='state'){
	...
	<div class="popup" id="distrreport"
		...
		<div class="pp-body">
			...
			<table id="businessTable" width="100%" height="60%"
				...
				<tbody id="businessInfo">
					<core:forEach items="${DistributionReports}" var="bean"
						varStatus="i">
						<tr class="odd">
							<td class="td_04"><div>
	...
	<div class="wrapper">
		...
		<div class="content_wrapper">
			...
			<div class="content" id="id_content">
				...
				<!-- // subheader -->
				<input type="hidden" name="uploadreportpopup"
					value="${uploadreportpopup }" id="uploadreportpopup">
				<spring:form method="post" modelAttribute="reportForm"
					id="reportsForm">
					<div class="inner_box">
						<!-- box -->
							...
							<div class="wide_column_wrapper report_container">
								<div class="description" id="id_report_01">
									<core:if test="${reportForm.reportRows == null}">
										...
										<p style="color: red">
											Purist Data is a restricted version of LBL Pro. To gain
											access to the full version of LBL Pro, please contact
											Convergent Mobile support, <a
												href="MAILTO:support@convergentmobile.com">support@convergentmobile.com</a>
										</p>
										</br>
										<%} %>
												...
												<tr class="row${rowidx}" id="tr_${loopStatus.index}">
													<core:forEach begin="0" end="${colCnt-1}" varStatus="i">
														...
														<core:if test="${i.index == 2}">
															<td class="td_02" id="date"><input type="hidden"
																value="${reportDataRow.field3}" id="datefieldvalue">
																<core:choose>

																	<core:when test="${reportType eq 'UploadReport' }">

																		<core:set value="${reportDataRow.field3}"
																			var="datefield" />
																		<fmt:parseDate value="${datefield }" var="datevalue"
																			pattern="yyyy-MM-dd HH:mm:ss" />
																		<fmt:formatDate value="${datevalue }" var="datevalue"
																			pattern="MM/dd/yyyy" />
																		<core:out value="${datevalue}" />
																	...
																	<core:otherwise>
																		...
																		<core:choose>
																			<core:when
																				test="${fn:containsIgnoreCase(template, 'Template')}">
																				<core:set var="templateReplace"
																					value="${fn:replace(template, 'Template', '')}"
																					scope="page" />
																				<core:out value="${templateReplace}" />
																			</core:when>
																			<core:when
																				test="${fn:contains(templateReplace, '_')}">
																				<core:set var="templateReplaceVal"
																					value="${fn:replace(templateReplace, '_', '')}"
																					scope="page" />
																				<core:out value="${templateReplaceVal}" />
																			</core:when>
																			<core:otherwise>
																				<core:out value="${reportDataRow.field3}" />
																			</core:otherwise>
						...
						<div class="c_icons_wrapper">
							<a href="reportxls.htm" class="lnk_convert_ico ico_xls">XLS</a> <a
								href="reportpdf.htm" class="lnk_convert_ico ico_pdf">PDF</a> <a
				...
				<div class="inner">
					...
					<div class="sb_title no-bg" style="min-width: 365px">
						...
						<div class="select_wrapper_01">
							<%if( Role==LBLConstants.PURIST ){ %>
							<spring:select class="" path="report.id"
								onchange="return getParams();" disabled="true">
					...
					<div class="sb_box_01 sb_box_h01">
								...
								<tbody>
									...
									<tr id="startdate">
										<td class="td_01"><label for="date_start">Start
												Date</label></td>
										<td class="td_02"><spring:input path="startDate"
												id="startDate" class="input_date_01" /></td>
									</tr>
									<tr id="enddate">
										<td class="td_01"><label for="date_end">End Date</label></td>
										<td class="td_02"><spring:input path="endDate"
												id="endDate" class="input_date_01" /></td>
									</tr>
										...
										<td class="td_02"><select name="client" id="clientName"
											disabled="disabled">
												...
												<core:forEach items="${clientNameInfo}" var="client">
													<core:choose>
														<core:when
															test="${client.brandName == reportForm.brandName}">
									...
									<tr id="gmbDrop" style="display: none">
										<td class="td_01"><label>Select Option</label></td>
										<td class="td_02"><select name="gmboption"
											id="gmboption">
									...
									<tr>
										<td class="td_01" style="display: none;" id="label"
											class="search"><label for="date_end">Search
												Store #</label></td>
										<td style="display: none;" id="brandidvalue1"><input
											type="text" name="store" id="storesvalue"></td>
										<td style="display: none;" id="searchbutton"><a href="#"
											style="margin: 0px -17px 0px" class="btn_search"><span
												onclick="search()">Search</span></a></td>
									</tr>

									<tr id="listingDrop">
										<td class="td_01"><label>Store #</label></td>
										<td class="td_02"><select class="select-width"
											name="StoreValues" id="stores_list_d" multiple="multiple"
											style="height: 100px">
									...
									<tr>
										...
										<td style="display: none;" id="clearsearch"><a href=""
											style="margin: 0px 327px 0px" onclick="">Clear</a></td>

									</tr>

									<tr id="storeDrop" style="display: none">
										<td class="td_01"><label>Listings Stores</label></td>
										<td class="td_02"><select name="storeVal"
											id="storevalue">
									...
									<tr id="directoryDrop" style="display: none">
										...
										<td class="td_02"><select name="directory"
											id="directory">
									...
									<tr id="stateDrop" style="display: none">
										<td class="td_01"><label>State</label></td>
										<td class="td_02"><select name="stateVal"
											id="statevalue">
									...
									<tr id="durationDrop" style="display: none">
										<td class="td_01"><label>Last</label></td>
										<td class="td_02"><select name="duration"
											id="duration">
						...
						<div class="button_wrapper_02">
								...
								<a href="#" id="runReport" onclick="runIt()"
									class="btn_dark_blue btn_report_01_lnk">Run Report</a> <a
									href="#" id="runcheckReport" onclick="runcheckreport()"
									style="display: none" class="btn_dark_blue btn_report_01_lnk">Run
									check Report</a> <a href="#" id="runaccuracyReport"
									onclick="runaccuracyreport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Accuracy Report</a>
								<a href="#" id="runCompareReport" onclick="runComparereport()"
									style="display: none" class="btn_dark_blue btn_report_01_lnk">Run
									Compare Report</a> <a href="#" id="runCitationReport"
									onclick="runCitationreport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Citation Report</a>
								<a href="#" id="runChangeTrackingReport"
									onclick="runChangeTrackingReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run ChangeTracking
									Report</a> <a href="#" id="runrenewalReport"
									onclick="runRenewalReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Renewal Report</a>

								<a href="#" id="runOverrideReport" onclick="runOverrideReport()"
									style="display: none" class="btn_dark_blue btn_report_01_lnk">Run
									Override Report</a> <a href="#" id="runSummaryReport"
									onclick="runSummaryReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Summary Report</a>
								<a href="#" id="runVislibilityReport"
									onclick="runVislibilityReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Visibility
									Report</a> <a href="#" id="runAccuracyStoreReport"
									onclick="runAccuracyStoreReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run AcccuracyStore
									Report</a> <a href="#" id="runCitationStoreReport"
									onclick="runCitationStoreReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run CitationStore
									Report</a> <a href="#" id="rungmbStoreReport"
									onclick="rungmbStoreReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Report</a> <a
									href="#" id="rungmbBrandReport" onclick="rungmbBrandReport()"
									style="display: none" class="btn_dark_blue btn_report_01_lnk">Run
									Report</a> <a href="#" id="rungmbMonthlyReport"
									onclick="rungmbMonthlyReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Report</a>
									
									 <a href="#" id="runDirectoryPerfLocationReport"
									onclick="runDirectoryPerfLocationReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Report</a>
									
									 <a href="#" id="runDirectoryPerfReport"
									onclick="runDirectoryPerfReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Report</a>
									
									
									<a href="#" id="rankingReport"
									onclick="rankingReport()" style="display: none"
									class="btn_dark_blue btn_report_01_lnk">Run Report</a>
